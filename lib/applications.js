...
class TCPClient extends Application {
  ...
  static isValid(obj) {
    ...
    if (obj.hasOwnProperty('maxBytes') && !is.number(obj.maxBytes)) {
      throw Error('expected number for maxBytes field');
    }
    if (obj.hasOwnProperty('packetSize') && !is.number(obj.packetSize)) {
      throw Error('expected number for packetSize field');
    }
    if (obj.hasOwnProperty('dataRate') && !is.dataRate(obj.dataRate)) {
      throw Error('expected number for dataRate field');
    }
    return true;
  }
}
...
class Ping extends Application {
  ...
  static isValid(obj) {
    ...
    if (obj.hasOwnProperty('interval') && !is.time(obj.interval)) {
      throw Error('interval has to be a time');
    }
  }
}
